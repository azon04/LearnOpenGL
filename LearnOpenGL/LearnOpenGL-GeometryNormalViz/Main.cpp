...
int main() {
	...
	Shader normalVizShader("Shaders/VisualNormal.vs", "Shaders/VisualNormal.frag", "Shaders/VisualNormal.gs");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// render normal
		normalVizShader.Use();
		normalVizShader.setMat4("view", view);
		normalVizShader.setMat4("projection", projection);
		...
		// render the loaded model
		{
			...
			model = glm::translate(model, glm::vec3(0.0f, -1.75f, 0.0f));
			model = glm::scale(model, glm::vec3(0.2f, 0.2f, 0.2f));
			normalVizShader.setMat4("model", model);
			ourModel.Draw(&shader);
		}
